...
```shell
1、操作系统：CentOS 7.7，最小化安装
系统初始化操作由 ansible-playbook 来完成如果通过其他方法提前完成。可不执行初始化相关的 playbook。
注意：
1) 以下命令是通过 root 用户完成。
2) 已2个 '#' 号开头的是注释内容
3) 已1个 '#' 号开头的是执行的命令
...
3、hosts 配置说明
每个主机下都会分发 hosts 文件，而这个文件依赖 /etc/ansible/hosts 中的 node_name 生成。
可在 /etc/ansible/hosts 中自定义修改。


3、 环境变量配置文件
## 放在与 roles 同级目录的 group_vars 下
...
这里有个问题，必须使用透传代理，不然无法正常使用，普通的 upstream 需要带上证书，会有一些列的麻烦。
注意，如果使用 haproxy，需要解决这个透传代理的问题。
...
3、分步部署
# ansible-playbook 01-common.yml    ## 环境初始化
# ansible-playbook 02-tls.yml       ## 签发相关组件的证书
# ansible-playbook 03-prepare.yml   ## 分发证书和工具
# ansible-playbook 04-etcd.yml      ## 部署 etcd 集群
# ansible-playbook 05-ha.tml        ## 部署 nginx 代理 + keepalived
# ansible-playbook 06-master.yml    ## 部署 master 集群
# ansible-playbook 07-node.yml      ## 部署 node 节点
# ansible-playbook 08-docker.yml    ## 部署 docker 容器
# ansible-playbook 09-network.yml   ## 部署 calico 网络组件

4、需要重新执行某个任务，可以单独添加 tags
...
3、重新分发 kubelet.service 文件，重启 kubelet 服务
# ansible-playbook --tags=restart_node_service 07-node.yml
...
6、再次手动 approve server cert csr
# kubectl get csr | grep Pending | awk '{print $1}' | xargs kubectl certificate approve
```
